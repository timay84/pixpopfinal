let config={toy:'ghost',actions:{},camera:false}; let hidden=false; let eligible=true; let toy; let eligibilityTimer;
const stage=document.getElementById('stage');
function toyMarkup(){ if(config.toy==='ghost')return '<div class="toy-art"><div class="ghost-aura"></div><img src="../assets/ghost-cutout.png" alt="幽灵杆"></div>'; if(config.toy==='radish')return '<div class="toy-art"><div class="knife"></div></div>'; return '<div class="toy-art"><div class="squeeze"></div></div>'; }
...
window.pixpop.loadConfig().then(c=>{config=c;toy=document.getElementById('toy');renderToy();setTimeout(show,700);});
...
window.pixpop.onOverlayCommand(command=>{if(command.type==='config'){config=command.config;renderToy();show();}});
